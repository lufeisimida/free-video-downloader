    ...
    <div v-if="visible" class="fixed inset-0 z-[100] flex items-center justify-center" @click.self="$emit('close')">
      ...
      <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden animate-modal-in">
        ...
        <form @submit.prevent="handleSubmit" class="px-6 sm:px-8 pb-8">
          ...
          <p v-if="registrationEnabled" class="mt-4 text-center text-sm text-text-secondary">
            {{ isLogin ? '还没有账号？' : '已有账号？' }}
...
watch(() => [props.visible, props.initialMode, props.registrationEnabled], ([visible]) => {
  if (!visible) return
  isLogin.value = props.registrationEnabled && props.initialMode === 'register' ? false : true
  error.value = ''
})
