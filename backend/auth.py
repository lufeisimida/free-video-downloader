...
def sync_builtin_account() -> dict:
    ...
    if not email or not password:
        raise RuntimeError(
            "请在 backend/.env 配置 BUILTIN_ACCOUNT_EMAIL 和 BUILTIN_ACCOUNT_PASSWORD"
        )
    if not validate_email(email):
        raise RuntimeError("BUILTIN_ACCOUNT_EMAIL 格式不正确")
    password_error = validate_password(password)
    if password_error:
        raise RuntimeError(f"BUILTIN_ACCOUNT_PASSWORD 不符合要求: {password_error}")
