...
from settings import is_registration_enabled, is_superuser_email
...
def _build_user_response(user: dict) -> dict:
    is_vip = is_superuser_email(user.get("email"))
    vip_expire_at = None
    if is_vip:
        vip_expire_at = "2099-12-31T23:59:59+00:00"
    elif user.get("is_vip") and user.get("vip_expire_at"):
        try:
...
async def get_auth_config():
    return {
        "success": True,
        "data": {"registration_enabled": is_registration_enabled()},
    }
...
async def register(req: RegisterRequest):
    ...
    return {
        ...
        "data": {
            "token": token,
            "user": _build_user_response({"id": user["id"], "email": req.email}),
        },
    }
